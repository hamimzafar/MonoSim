...
def main(num_clones,num_cells,num_mutations,fp_rate):
    #Take out spaces in the genome
    chr20 = open("chr20.fa","r")
    new_genome = chr20.read()
    ...
    for i in range(len(matrix)):
        ...
        f.write("\n")
    f.close()
    amps = amplify_genome(sampled, fp_rate, 10)
        ...
        for unit in amps[cell]:
            for amp in amps[cell][unit]:
                for idx in range(len(amps[cell][unit][amp])):
                    ...
                    #print len(fasta)
                    #print "fasta",fasta
                    GenReads(fasta=fasta, length=100, qual=33, models=err_file,
                             out = str(cell))
...
main(num_clones= 10, num_cells= 100, num_mutations= 50, fp_rate = 10**(-6))